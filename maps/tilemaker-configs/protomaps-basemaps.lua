...
node_keys = { "amenity","barrier","highway","historic","leisure","natural","office","place","railway","shop","sport","tourism","waterway" }
...
-- On zoom 12, various road classes are merged into "minor_road"
...
landcoverKeys   = { forest="forest", wood="forest",
                    wetland="wetland",
                    ...
                    fell="garden", grassland="garden", grass="garden", heath="garden", meadow="garden", allotments="garden", park="garden", village_green="garden", recreation_ground="garden", scrub="garden", shrubbery="garden", tundra="garden", garden="garden", golf_course="garden", park="garden" }
...
-- POI "kind" values: based on https://github.com/openmaptiles/openmaptiles/blob/master/layers/poi/poi.yaml
...
function write_to_transportation_layer(minzoom, highway_class, subclass, ramp, service, is_rail, is_road, is_area)
	Layer("roads", is_area)
	SetZOrder()
	Attribute("kind", highway_class)
	AttributeNumeric("sort_rank", 400)
	if subclass and subclass ~= "" then
		Attribute("kind_detail", subclass)
	end
...
function way_function()
	...
	if aerialway ~= "" then
		...
		if HasNames() then
			...
			Attribute("kind_detail", aerialway)
		end
	end
	...
	if highway ~= "" or public_transport == "platform" then
		...
		if minzoom <= 14 then
			...
			if not is_closed and (HasNames() or Holds("ref")) then
				...
				elseif h == "trunk" then
					minzoom = 8
				elseif h == "major_road" or h == "primary" then
					minzoom = 10
				elseif h == "secondary" then
					minzoom = 11
				elseif h == "minor_road" or h == "track" or h == "tertiary" then
					minzoom = 13
				else
					minzoom = 14
				end
				...
				if subclass then Attribute("kind_detail", highway) end
				local ref = Find("ref")
				if ref~="" then
					Attribute("ref",ref)
	...
	if waterwayClasses[waterway] and not is_closed then
		...
		SetNameAttributes()
		SetBrunnelAttributes()
	elseif waterway == "boatyard"  then Layer("landuse", is_closed); Attribute("kind", "industrial"); MinZoom(12)
	elseif waterway == "dam"       then Layer("buildings",is_closed); Attribute("kind", "building")
	elseif waterway == "fuel"      then Layer("landuse", is_closed); Attribute("kind", "industrial"); MinZoom(14)
	end
	-- Set names on rivers
	if waterwayClasses[waterway] and not is_closed then
		if waterway == "river" and Holds("name") then
	...
	if landcoverKeys[l] then
		Layer("landuse", true)
		SetMinZoomByArea()
		Attribute("kind", landcoverKeys[l])
		if l=="wetland" then Attribute("kind_detail", Find("wetland"))
		else Attribute("kind_detail", l) end
		write_name = true
		Attribute("sort_rank", 189)
...
function WritePOI(class,subclass,rank)
	local layer = "pois"
	if rank>4 then layer="pois_detail" end
...
function SetBrunnelAttributes()
	...
	elseif Find("tunnel") == "yes" then AttributeBoolean("is_tunnel", true)
...
-- Calculate POIs (typically rank 1-4 go to 'poi' z12-14, rank 5+ to 'pois_detail' z14)
